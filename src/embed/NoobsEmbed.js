...
class NoobsEmbed extends MessageEmbed {
  constructor(config = {}) {
    ...
    if (config.type === "book") {
      ...
      this.addReactionHandler("◀️", (reaction, user) => {
  ...
  addPage(embed = new MessageEmbed()) {
    this.pages.set(this.pages.length, embed);
    return this;
  }
  addPages(embeds = []) {
    if (embeds.length) embeds.map((e) => this.pages.set(this.pages.length, e));
    return this;
  }
  addReactionHandler(emoji, fn) {
    const target = this.client.emojis.resolveIdentifier(emoji);
    this.handlers.set(target, fn);
    return this;
  }
  send(config = {}) {
    const { msg, options } = config;
    const filter = (reaction, user) =>
      this.handlers.has(reaction.emoji.identifier) && user.id === msg.author.id;

    const collect = (reaction, user) => {
      const fn = this.handlers.get(reaction.emoji.identifier);
      if (handler) {
        reaction.users.remove(user).catch(console.error);
    ...
    const end = () => msg.reaction.removeAll();

    this.#book = msg.channel
      .send(this.pages.get(this.page))
    ...
    return [...this.handlers.keys()].map((e) => this.#book?.message?.react(e));
  }
}
